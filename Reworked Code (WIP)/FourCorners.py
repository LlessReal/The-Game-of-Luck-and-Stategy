...
def FourCorners():
    ...
    while True:
        try:
            print("Pick your color."); sleep(0.5)
            for x in range(1,5): print(f"\n{x} - {AllColors[x]}"); sleep(0.25)
            You = NumChoiceWithQuit(range(1,5)) - 1 # -1 for the index
            if You == "Quit": return
            elif You == "Redo": continue
        except: InvalidInput(); continue
        ...
        print("The color chosen is......"); sleep(0.5)
        print(f"{AllColors[FourColors]}!!!"); sleep(0.5)
...
def FourCorners(OnePlayerName,TwoPlayerName):
    ...
    while True:
        ...
        try:
            clearScreen()
            print(f"{OnePlayerName}! Choose your color."); sleep(0.5)
            for x in range(1,5): print(f'''\n {x} - {AllColors[x]}'''); sleep(0.25)
        ...
        while True:
            try:
                ...
                print(f"{TwoPlayerName}! Choose your color.");  sleep(0.5)
                for x in range(1,5): print(f'''\n {x} - {AllColors[x]}'''); sleep(0.25)
                TwoPlr = NumChoiceWithQuit(range(1,5))
                if TwoPlr == "Quit": return
                elif TwoPlr == "Redo": continue
            except: InvalidInput();  continue
            ...
            print("The color chosen is......"); sleep(0.5)
            print(f"{AllColors[FourColors - 1]}!!!"); sleep(0.5)
            for Character in AllPlayers:
                if Character[1] > 0:
                    if Character[0] != FourColors:
                        ...
                        if Character[1] == 0: PlayersOut += 1
                MessageStop()
                if YourLives > 0 and TwoPlrLives == 0:
                    MessageStop(f"{OnePlayerName} has lasted longer than {TwoPlayerName} in Four Corners!"); return "W"
                elif YourLives == 0 and TwoPlrLives > 0:
                    MessageStop(f"{TwoPlayerName} has lasted longer than {OnePlayerName} in Four Corners!"); return "L"
                elif YourLives == 0 and TwoPlrLives == 0:
